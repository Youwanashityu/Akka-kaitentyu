...
public class SoundManager : MonoBehaviour
{
    ...
    [Header("VC用 AudioSource")]
    [SerializeField] private AudioSource vcSource;
    ...
    [Header("VC 一覧")]
    [SerializeField] private List<VoiceData> vcList = new();

    private Dictionary<string, VoiceData> vcDict;
    ...
    private void Awake()
    {
        ...
        // VC辞書を構築
        vcDict = new Dictionary<string, VoiceData>();
        foreach (var v in vcList)
        {
            if (!string.IsNullOrEmpty(v.name) && !vcDict.ContainsKey(v.name))
                vcDict.Add(v.name, v);
        }
    }
    ...
    /// 名前を指定してVCを再生します。
    /// </summary>
    /// <param name="name">VoiceDataに設定したボイス名</param>
    public void PlayVC(string name)
    {
        if (!vcDict.TryGetValue(name, out var data))
        {
            Debug.LogWarning($"[SoundManager] VC '{name}' が見つかりません。");
            return;
        }

        vcSource.clip = data.clip;
        vcSource.volume = data.volume;
        vcSource.Play();
    }
}